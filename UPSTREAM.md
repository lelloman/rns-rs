...
- Normative commit: `42f6d64b9cc379aad73a6a8927dadec91f7031d7`
- Commit date: `2026-08-21 16:48:05 +0200`
- Subject: `Implemented per-interface protocol violation tracking`
...
`b1dca84133623eff6cb13fbf984bce888be7a4b1`. The CI interop lane remains pinned
...
`42f6d64b9cc379aad73a6a8927dadec91f7031d7`. The first commit fixes
...
regression verifies every deregistration. The twelfth commit clears stale
Android BLE discovery state and is non-runtime here because this port has no
Android/Bluetooth RNode client. The thirteenth commit corrects a Python `rncp`
...
non-runtime. The nineteenth commit prevents proof callbacks from running while
Python holds its receipt-list lock. Rust owns receipt state on the driver thread
and removes the tracked send before invoking `on_proof`; a focused regression
now proves that the callback can synchronously enqueue and dispatch another
outbound message. The twentieth commit adds only upstream TODO comments about
...
and retains its endpoint-safety regression. The twenty-seventh commit removes
obsolete Python queue code and routes the tagless path-request diagnostic to
the pathing log level. Rust has no obsolete queue block and now emits the
equivalent diagnostic through its dedicated pathing target. The twenty-eighth
commit lowers the default class capacities and makes data, announce,
path-request, and ingress-limited lengths independently configurable. Rust
...
absent. The thirty-first commit adds only a TODO proposing future early
...
correct. The thirty-fourth commit moves successful authenticated link-path
rebalance diagnostics to the dedicated pathing log level; Rust now emits the
equivalent event on its pathing target. The thirty-fifth commit adds cumulative
per-class queue-drop counters to runtime statistics and displays nonzero counts
...
announce and path-request pressure from their independent fields. The
thirty-eighth commit repairs a Python persistence-loop cache timestamp; Rust
...
without caches. The forty-third commit fixes upstream's discovery-tag retention
slice to keep the newest entry; Rust's bounded FIFO already retains exactly the
configured capacity and now asserts the complete surviving set. The
...
has an implementation-specific name and needs no rename. The forty-eighth
commit aligns Resource part matching with the requested window after the
completed height. Rust had the same off-by-one search and now starts at the
next part, covered at the inclusive tail of an out-of-order receive window. The
...
chunk test. The fifty-second commit applies the same correction to Python
`rnsh`; native `rnsh` already uses `Link MDU - channel envelope - stream header`
...
The sixty-eighth commit distinguishes all link-table entries from validated or
established active links in `rnstatus`. Rust preserves the existing total-count
...
The seventieth commit tracks protocol, IFAC, and ordinary packet-filter
rejections per interface. Rust records malformed frames, IFAC mismatches and
unexpected flags, invalid announces, tagless path requests, duplicate/filter
hits, and pre-validation link traffic; prevents the latter from forwarding;
aggregates listener children; and exposes sortable `rnstatus` counters.
