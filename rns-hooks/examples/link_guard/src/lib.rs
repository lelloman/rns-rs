...
use rns_hooks_sdk::context::{self, LinkContext};
use rns_hooks_sdk::host;
use rns_hooks_sdk::result::HookResult;

/// Maximum link requests allowed within the sliding window.
...
pub extern "C" fn on_hook(ctx_ptr: i32) -> i32 {
    ...
    let result = if ctx_type == context::CTX_TYPE_LINK {
        let link = unsafe { &*(ptr as *const LinkContext) };

        // Check if this link source is blackholed
        if host::is_blackholed(&link.link_id) {
            host::log_str("link_guard: dropping blackholed link request");
            ...
            unsafe {
                ...
                if COUNTER > MAX_REQUESTS {
                    host::log_str("link_guard: rate limiting link requests");
                    HookResult::drop_result()
                } else {
